...
class FieldElement:
    ...
    k_modulus = 3 * 2**30 + 1
    generator_val = 5
    byte_size = ceil(log2(k_modulus)) // 8
    ...
    def to_bytes(self) -> bytes:
        assert self.val >= 0
        return self.val.to_bytes(self.byte_size, 'big')

    @classmethod
    def from_bytes(cls, value: bytes) -> 'FieldElement':
        return FieldElement(int.from_bytes(value, 'big'))
