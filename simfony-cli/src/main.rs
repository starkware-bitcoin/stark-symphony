...
fn handle_build(path: PathBuf, witness: Option<PathBuf>, output_path: Option<PathBuf>) -> Result<()> {
...
fn handle_run(path: PathBuf, witness: Option<PathBuf>, param: Option<PathBuf>) -> Result<()> {
    ...
    let param_content = if let Some(param_path) = param {
        Some(fs::read_to_string(&param_path)
            .with_context(|| format!("Failed to read parameter file: {}", param_path.display()))?)
    } else {
        None
    };

    let witness_content = if let Some(witness_path) = witness {
        Some(fs::read_to_string(&witness_path)
            .with_context(|| format!("Failed to read witness file: {}", witness_path.display()))?)
    } else {
        None
    };
...
fn main() {
    ...
    let result = match cli.command {
        Commands::Build { path, witness, output_path } => {
            handle_build(path, witness, output_path)
        },
        Commands::Run { path, witness, param } => {
