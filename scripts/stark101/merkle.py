...
class MerkleTree(object):
    ...
    def __init__(self, data: List[FieldElement]):
    ...
    def get_authentication_path(self, leaf_id: int) -> List[bytes]:
    ...
    def recursive_build_tree(self, node_id: int):
        if node_id >= len(self.data):
            # A leaf.
            id_in_data = node_id - len(self.data)
            leaf_data = self.data[id_in_data].to_bytes()
            h = sha256(leaf_data).digest()
...
def verify_decommitment(leaf_id: int, leaf_elt: FieldElement, decommitment: List[bytes], root: bytes):
    leaf_num = 2 ** len(decommitment)
    node_id = leaf_id + leaf_num
    cur = sha256(leaf_elt.to_bytes()).digest()
